...
const matchUpFormatCode = /*#__PURE__*/function () {
  const fx = {};

  fx.stringify = matchformatobject => {
    if (matchformatobject && typeof matchformatobject === "object") {
      if (matchformatobject.timed && !isNaN(matchformatobject.minutes)) return timedSetFormat(matchformatobject);
  ...
  function getSetFormat(matchformatobject) {
    ...
    const finalSetCode = bestOfValue > 1 && finalSetCountValue && !finalSetCountValue.invalid && `F:${finalSetCountValue}` || "";
    const valid = bestOfCode && setCountValue && !setCountValue.invalid && (!finalSetCountValue || !finalSetCountValue.invalid);
  ...
  fx.parse = matchformatcode => {
    if (matchformatcode && typeof matchformatcode === "string") {
      const type = matchformatcode.indexOf("T") === 0 ? "timed" : matchformatcode.indexOf(SET) === 0 ? SET : "";
    ...
    if (formatstring && formatstring[1] === ":") {
      ...
      if (setType && setFormatString) {
        ...
        if (tiebreakSet) return {
          tiebreakSet: parseTiebreakFormat(setFormatString)
        };
...
//# sourceMappingURL=tods-matchup-format-code.cjs.development.js.map
